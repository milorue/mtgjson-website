  ...
  async created() {
    try {
      ...
      if (limitJson.rate.remaining > 0) {
        const fetchedApp = await fetch(
          "https://api.github.com/repos/mtgjson/mtgjson/contributors"
        );
        const appJson = await fetchedApp.json();
        const fetchedDocs = await fetch(
          "https://api.github.com/repos/mtgjson/mtgjson-website/contributors"
        );
        const docsJson = await fetchedDocs.json();

        const contribs = await [...appJson, ...docsJson];

        // Only return unique contributors
        return (this.contributors = Array.from(
          new Set(contribs.map((u) => u.login))
        ).map((login) => {
          return {
            login,
...
<style lang="scss">
.home {
  padding: var(--navbar-height) 4rem 4rem var(--sidebar-width);
  display: block;
